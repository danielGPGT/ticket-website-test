...
async function fetchSport(sportType: string) {
	...
	// Map "soccer" to "football" for sports table lookup
	const sportKey = sportType.toLowerCase() === "soccer" ? "football" : sportType.toLowerCase();
...
export default async function FootballEventDetail(props: { params: { eventSlug: string } | Promise<{ eventSlug: string }> }) {
	...
	return <EventDetailContent event={event} tickets={tickets} categories={categories} sportPath="/football" tournament={tournament} sport={sport} />;
}
