...
type EventsExplorerEnhancedProps = {
	hiddenFilters?: string[]; // Array of filter section IDs to hide
};
...
export function EventsExplorerEnhanced({ hiddenFilters = [] }: EventsExplorerEnhancedProps) {
	...
	// Helper to get base path for navigation (sport page or /events)
	const getBasePath = useMemo(() => {
		if (pathname && pathname !== "/events" && (
			pathname.startsWith("/formula-1") || 
			pathname.startsWith("/tennis") || 
			pathname.startsWith("/football") || 
			pathname.startsWith("/motogp")
		)) {
			return pathname;
		}
		return "/events";
	...
	// Get sport type from pathname if on a sport page
	const sportFromPathname = useMemo(() => {
		if (pathname === "/formula-1") return "formula1";
		if (pathname === "/tennis") return "tennis";
		if (pathname === "/football") return "football";
		if (pathname === "/motogp") return "motogp";
		return null;
	}, [pathname]);
	...
	const filters: FilterState = useMemo(() => {
		...
		// If on a sport page and no sport_type in URL, use the pathname sport
		const sportTypeParam = getArrayParam("sport_type");
		const defaultSportType = sportFromPathname && sportTypeParam.length === 0 ? [sportFromPathname] : sportTypeParam;
	...
	useEffect(() => {
		...
		Promise.all(requests)
			...
			.catch((error) => {
				...
				setError("Failed to load events. Please try again or adjust your filters.");
				setAllEvents([]);
			})
	...
	const handleFilterChange = (newFilters: FilterState) => {
		...
		if (newFilters.sportType.length > 0) {
			const sportMatchesPage = isSportPage && newFilters.sportType.length === 1 && newFilters.sportType[0] === sportFromPathname;
			if (!sportMatchesPage) {
		...
		<div className="relative">
			<div className="flex flex-col lg:flex-row gap-6">
				...
				<aside className="hidden lg:block w-80 shrink-0">
					<EventsFilters onFilterChange={handleFilterChange} initialFilters={filters} events={allEvents} hiddenFilters={hiddenFilters} />
				</aside>
						...
						<CardContent className="flex flex-col items-center justify-center py-20 text-center">
							...
							<p className="text-sm text-muted-foreground mb-6 max-w-md">
								{filters.sportType.length === 0 && filters.tournamentId.length === 0 && filters.countryCode.length === 0 && !teamId && !filters.query && !filters.dateFrom && !filters.dateTo
									? "Get started by selecting a sport, competition, or country from the filters, or search for a specific event."
									: activeFilterCount > 0
									? `No events match your ${activeFilterCount} filter${activeFilterCount > 1 ? "s" : ""}. Try adjusting your filters or expanding your search criteria.`
							...
							<div className="flex gap-2 flex-wrap justify-center">
								{activeFilterCount > 0 && (
									<Button
										variant="outline"
										onClick={() => {
											const basePath = pathname && pathname !== "/events" && (pathname.startsWith("/formula-1") || pathname.startsWith("/tennis") || pathname.startsWith("/football") || pathname.startsWith("/motogp"))
												? pathname
												: "/events";
								...
								<Button
									variant="default"
									onClick={() => {
										// On sport pages, go to /events for "all events", otherwise add origin param
										const isSportPage = sportFromPathname !== null;
										const targetUrl = isSportPage ? "/events?origin=allevents" : `${getBasePath}?origin=allevents`;
			...
			<Drawer open={mobileFiltersOpen} onOpenChange={setMobileFiltersOpen}>
				...
				<DrawerContent className="!max-h-[90vh] p-0 bg-card">
					<div className="overflow-y-auto">
						<EventsFilters onFilterChange={handleFilterChange} initialFilters={filters} events={allEvents} isMobile={true} hiddenFilters={hiddenFilters} />
					</div>
				</DrawerContent>
			</Drawer>
		</div>
